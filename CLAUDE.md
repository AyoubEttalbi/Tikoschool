...
> That file is tracked; keep CLAUDE.md generic and put school-specific overrides there.
...
- **Cloudinary uploaders were removed** (profile-image feature, Aug 2026): the three copy-pasted `getCloudinary()` uploaders with their runtime `env()` reads are gone, replaced by `ProfileImageService` (local private WebP storage); the `cloudinary/cloudinary_php` package was dropped. Legacy `res.cloudinary.com` URLs in old rows still render via `ProfileImageUrl::resolve()`. The gap 'uploads fail in production under config:cache' is retired for profile images; any future file-upload feature must keep disk/root definitions in `config/filesystems.php`, never runtime `env()`.
