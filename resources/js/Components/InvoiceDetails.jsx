...
import { AlertCircle, Calendar, CheckCircle, ChevronDown, ChevronUp, CreditCard, DollarSign, Download, FileText, User, X } from 'lucide-react';
...
const InvoiceDetails = ({ invoice, onClose }) => {
  ...
  const paymentStatus = invoice.rest <= 0 ? 'paid' : 
                       invoice.amountPaid <= 0 ? 'unpaid' : 'partial';
  ...
  const remainingAmount = typeof invoice.rest === 'number' ? invoice.rest : (totalAmount - amountPaid);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {invoice.includePartialMonth && (
              <div className="col-span-2">
                <p className="text-gray-500 text-sm">Partial Month</p>
                <p className="font-medium">
                  Yes - {formatCurrency(invoice.partialMonthAmount)}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-4">
            ...
            <div className="mb-2">
              <div className="flex justify-between text-xs text-gray-600 mb-1">
                ...
                <span>{Number.isFinite(paymentPercentage) ? paymentPercentage : 0}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2.5">
                <div 
                  ...
                  style={{ width: `${Number.isFinite(paymentPercentage) ? paymentPercentage : 0}%` }}
                ></div>
              </div>
            </div>
          </div>
          ...
          {invoice.payments && invoice.payments.length > 0 && (
            <div>
              ...
              >
                <span>View Payment History ({invoice.payments.length})</span>
                  ...
                  {invoice.teachers.map((teacher, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-600">
                        {formatCurrency(teacher.amount * (invoice.months || 1))}
                      </td>
                    </tr>
                  ))}
                </tbody>
