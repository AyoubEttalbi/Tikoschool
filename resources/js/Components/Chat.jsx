  ...
  return (
    <div className={`flex mb-4 ${isUser ? 'justify-end' : 'justify-start'}`}>
      {!isUser && (
        <div className="mr-2">
          <Avatar 
            src={sender.avatar || "/chatprofile.jpg"} 
          />
        </div>
      )}
...
const OnlineUser = ({ user, onClick }) => {
  return (
    <div className="text-center m-3 cursor-pointer" onClick={onClick}>
      ...
      <p className="text-xs font-medium mt-1 truncate max-w-[60px] mx-auto">{user.name}</p>
    </div>
  );
};
  ...
  const [messagesByContact, setMessagesByContact] = useState({
    1: [
      ...
      },
      {
        ...
        message: "Pretty good, thanks for asking! Working on some new designs.",
        time: "10:25 AM",
        isUser: true,
        isRead: true
      },
    ...
    3: [
      {
        ...
        message: "Hi there! I've reviewed the project requirements and have some thoughts to share.",
        time: "10:23 AM",
        isUser: false,
        isRead: true
      },
      {
        id: 2,
        sender: { name: "You", avatar: "/chatprofile.jpg" },
        message: "Great! I'm particularly interested in your thoughts about the timeline.",
        time: "10:25 AM",
        isUser: true,
        isRead: true
      },
      {
        id: 3,
        sender: { name: "Danny McChain", avatar: "/chatprofile.jpg" },
        message: "I think we need to allocate more time for the testing phase. Quality assurance is critical for this client.",
        time: "10:28 AM",
        isUser: false,
        isRead: true
      },
      {
        id: 4,
        sender: { name: "You", avatar: "/chatprofile.jpg" },
        message: "I agree. How much additional time would you recommend?",
        time: "10:30 AM",
        isUser: true,
        isRead: true
      }
          ...
          <div className="flex flex-row flex-1 overflow-hidden">
            ...
            <div className="w-1/3 border-r border-gray-200 flex flex-col">
              ...
              <div className="p-3 border-b border-gray-200 bg-gray-50">
                <h3 className="text-xs font-semibold text-gray-600 mb-2 px-1">ONLINE NOW</h3>
            ...
            <div className="w-2/3 flex flex-col">
              ...
              {activeContact && (
                <div className="px-4 py-3 border-b border-gray-200 flex items-center bg-white">
                  <Avatar 
                    src={contacts.find(c => c.id === activeContact)?.avatar || "/chatprofile.jpg"} 
                    status={contacts.find(c => c.id === activeContact)?.status}
                  />
                  <div className="ml-3">
                    <p className="font-medium text-gray-800">
                      {contacts.find(c => c.id === activeContact)?.name || "Select a contact"}
                    </p>
                    <p className="text-xs text-gray-500">
                      ...
                      {contacts.find(c => c.id === activeContact)?.status === "busy" && (
                        <span className="text-red-500">Busy</span>
                      )}
                      {contacts.find(c => c.id === activeContact)?.status === "offline" && (
                        <span className="text-gray-500">Last seen 2h ago</span>
                      )}
                    </p>
                  </div>
              ...
              <div className="flex-grow p-4 overflow-y-auto bg-gray-50">
                {activeContact && messagesByContact[activeContact] ? (
                  messagesByContact[activeContact].map(message => (
                    <ChatMessage
                      key={message.id}
...
const ChatDropdown = ({ isOpen, onClose, buttonRef, onOpenFullChat, onContactSelect }) => {
  ...
  useEffect(() => {
    if (isOpen && buttonRef.current) {
      ...
      setPosition({
        top: rect.top + window.scrollY - (dropdownRef.current ? dropdownRef.current.offsetHeight : 0),
  ...
  const onlineUsers = [
    {
      id: 1,
      name: "Marie Horwitz",
      avatar: "/chatprofile.jpg",status: "online"
    },
    {
      id: 3,
      name: "Danny McChain",
              ...
              >
                <div className="flex items-center">
                  ...
                  <div className="ml-3 flex-grow overflow-hidden">
                    ...
                    <p className={`text-sm truncate ${message.unread ? 'font-semibold text-gray-800' : 'text-gray-500'}`}>
                      {message.message}
                    </p>
                  </div>
                  {message.unread && (
