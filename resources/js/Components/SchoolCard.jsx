    ...
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0 md:mt-4">
      <div className="flex items-center justify-between mb-8">
        <div className="flex flex-row md:flex-row items-center gap-4 w-full md:w-auto">
          <h1 className="font-semibold text-2xl">Schools List</h1>
          {role === "admin" && <FormModal table="school" type="create" />}
        </div>
      </div>
      <div className="container mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {schoolsData.map((school) => (
            <SchoolCard key={school.id} school={school} onDelete={setDeleteSchool} />
          ))}
        </div>
      </div>
