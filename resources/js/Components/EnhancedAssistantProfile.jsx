...
const EnhancedAssistantProfile = ({ assistant, performance, resources, communications, professional, students, schedule, documents, achievements }) => {
...
const PerformanceAnalytics = ({ data }) => {
  const performanceData = {
    labels: ['Teaching Rating', 'Attendance Rate', 'Lesson Completion', 'Student Progress'],
    datasets: [{
      data: [data.rating, data.attendance, data.lessonCompletion, data.studentProgress],
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <p className="text-2xl font-bold text-blue-600">{data.rating}/5</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="font-medium">Attendance Rate</h3>
            <p className="text-2xl font-bold text-green-600">{data.attendance}%</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="font-medium">Lesson Completion</h3>
            <p className="text-2xl font-bold text-yellow-600">{data.lessonCompletion}%</p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="font-medium">Student Progress</h3>
            <p className="text-2xl font-bold text-purple-600">{data.studentProgress}%</p>
          </div>
        </div>
      </div>
    </div>
          ...
          <div className="space-y-4">
            {data.lessonPlans.map((plan, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                <span>{plan.name}</span>
                <Link href={plan.url} className="text-blue-600 hover:underline">
                  View
                </Link>
              </div>
            ))}
          ...
          <div className="space-y-4">
            {data.sharedResources.map((resource, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                ...
                <Link href={resource.url} className="text-blue-600 hover:underline">
                  Access
                </Link>
              </div>
            ))}
          ...
          <div className="space-y-4">
            {data.certifications.map((cert, index) => (
              <div key={index} className="p-3 bg-gray-50 rounded">
                ...
                <p className="text-sm text-gray-600">Year: {cert.date}</p>
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-medium mb-4">Training Opportunities</h3>
          <div className="space-y-4">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-medium mb-4">Attendance Patterns</h3>
          <div className="h-64">
...
const DocumentManagement = ({ data }) => {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-medium mb-4">Important Documents</h3>
          <div className="space-y-4">
          ...
          <div className="space-y-4">
            {data.awards.map((award, index) => (
              <div key={index} className="p-3 bg-gray-50 rounded">
                ...
                <p className="text-sm text-gray-600">Year: {award.date}</p>
              </div>
            ))}
          </div>
        </div>
