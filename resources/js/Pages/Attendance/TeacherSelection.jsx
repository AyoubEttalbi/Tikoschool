...
import { Search, ChevronDown, Filter, Users, BookOpen, School } from "lucide-react";
import { Link, usePage } from "@inertiajs/react";

const TeacherSelection = ({ teachers, currentSelection, assistants, levels, schools }) => {
  ...
  const assistant = user.role === "assistant" 
    ...
    : null;
  const availableSchools = user.role === "assistant" && assistant?.schools 
    ? assistant.schools 
    : schools;
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600 mt-1">Choose class to view attendance records</p>
        </div>
          ...
          {teacher.classes.map((cls) => (
            ...
            >
              ...
              <div className="text-sm text-gray-600 ml-9">
                {levels.find((l) => l.id === cls.level_id)?.name}
              </div>
            </Link>
          ))}
        </div>
  ...
  const filteredTeachers = teachers.filter((teacher) => {
    ...
    const schoolMatch = !filters.school || teacher.schools.some((s) => s.id === filters.school);
    const levelMatch = !filters.level || teacher.classes.some((c) => c.level_id === filters.level);
    const assistantSchoolMatch =
      user.role !== "assistant" ||
      (assistant && teacher.schools.some((s) => assistant.schools.some((as) => as.id === s.id)));
              ...
              <div className="w-full md:w-auto">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setFilters({ 
                      ...filters, 
                      school: value ? Number(value) : null 
                    });
                  }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                      <School size={14} />
                      <span>{teacher.schools.map((s) => s.name).join(", ")}</span>
                    </div>
                  </div>
                  <ChevronDown
                    size={20}
                    className={`text-gray-400 transition-transform ${
                      expandedTeachers[teacher.id] ? "rotate-180" : ""
                    }`}
                  />
                </div>
                    ...
                    {teacher.classes.map((cls) => (
                      <Link
                        ...
                        className={`block p-3 rounded-md border transition-colors ${
                          currentSelection.teacher === teacher.id &&
                          currentSelection.class === cls.id
                            ? "border-indigo-300 bg-indigo-50"
