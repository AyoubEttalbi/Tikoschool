          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                {filteredAnnouncementsCount === totalAnnouncementsCount 
                  ? `Showing all ${totalAnnouncementsCount} announcements` 
                  : `Showing ${filteredAnnouncementsCount} of ${totalAnnouncementsCount} announcements`}
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['all', 'mine', 'teacher', 'assistant'].map(visibility => (
                    <button
                      key={visibility}
                    ...
                    >
                      {visibility === 'all' ? 'Everyone' : 
                       visibility === 'mine' ? 'For Me' :
                       visibility.charAt(0).toUpperCase() + visibility.slice(1) + 's'}
                    </button>
                  ))}
                </div>
              </div>
      ...
      <div className="divide-y divide-gray-100">
        {filteredAnnouncements.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="mt-3 text-base font-medium text-gray-900">No announcements found</h3>
            <p className="mt-1 text-sm text-gray-500">
              Try adjusting your filters or check back later for new announcements.
            </p>
            {(filters.status !== 'all' || filters.visibility !== 'all' || filters.search !== '') && (
              <button 
                onClick={resetFilters}
          ...
          filteredAnnouncements.map((announcement, index) => {
            ...
            const daysRemaining = getDaysRemaining(announcement.date_end);
            
            return (
              <div 
                key={announcement.id} 
                className={`px-6 py-5 hover:bg-gray-50 transition-colors duration-150 ${
                  index === filteredAnnouncements.length - 1 ? 'rounded-b-xl' : ''
                }`}
              >
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1.5">
                      <h2 className="font-semibold text-lg text-gray-800">{announcement.title}</h2>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`inline-flex items-center space-x-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${visibility.className}`}>
                          {visibility.icon}
                    ...
                    <div className="flex-shrink-0 w-full md:w-auto">
                      <div className={`flex flex-col items-center justify-center p-3 rounded-lg ${
                        ...
                        <div className="flex items-end mt-1">
                          ...
                          <span className="text-xs ml-1 mb-0.5">day{daysRemaining !== 1 ? 's' : ''}</span>
                        </div>
                      </div>
                    </div>
                  )}
