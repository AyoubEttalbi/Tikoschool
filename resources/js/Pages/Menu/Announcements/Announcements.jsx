...
const Announcements = ({ announcements = [], userRole = 'all', limit = 3 }) => {
    ...
    return [...announcements]
      .filter(announcement => {
        ...
        const isWithinDateRange = 
          (!announcement.date_start || new Date(announcement.date_start) <= now) &&
          (!announcement.date_end || new Date(announcement.date_end) >= now);
  ...
  const getVisibilityStyles = (visibility) => {
    ...
    return styles[visibility] || { bg: 'bg-gray-50', border: 'border-gray-200', icon: 'text-gray-500' };
  };
          ...
          return (
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between">
                <span className={`text-xs flex items-center ${styles.icon}`}>
                  <UsersIcon className="h-3 w-3 mr-1" />
                  {announcement.visibility === 'all' ? 'Everyone' : 
                   announcement.visibility.charAt(0).toUpperCase() + announcement.visibility.slice(1)}
                </span>
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-100">
        ...
        <div className="flex items-center space-x-3">
          <span className="text-xs px-2 py-1 bg-gray-100 rounded-full text-gray-600 font-medium">
            {userRole === 'all' ? 'All Users' : userRole.charAt(0).toUpperCase() + userRole.slice(1)}
          </span>
