...
class MembershipStatsService
{
    ...
    public function getOrCreateMonthlyStats($schoolId = null, $year = null, $month = null): MembershipMonthlyStats
    {
        ...
        // Read-through with a short TTL, instead of recomputing AND rewriting on every load.
        //
        // This method is called from StatsController on every Dashboard render, and it
        // used to unconditionally call updateMonthlyStats() — an unindexed scan over
        // memberships with whereHas('student'), iterated in PHP, followed by a SYNCHRONOUS
        // updateOrCreate() write to membership_monthly_stats. That table exists precisely
        // to be a cache, and the cache was never read: getMonthlyStats() was dead code
        // from the dashboard's perspective, and the dashboard wrote to the cache table on
        // every page view.
        //
        // The original comment justified this as "always calculate fresh stats… ensures
        // month filtering works correctly". That is preserved: a row is still recomputed
        // when it is missing or older than the TTL, so a newly-filtered month always gets
        // real numbers. What changes is that a second dashboard load inside the window
        // reads the row instead of rebuilding and rewriting it.
        //
        // The scheduled `memberships:update-stats --all` (daily at 01:00) owns the write
        // path for the general case.
        $cached = $this->getMonthlyStats($schoolId, $year, $month);

        $ttlMinutes = (int) config('monitoring.stats_cache_minutes', 10);

        if ($cached && $cached->updated_at && $cached->updated_at->gt(now()->subMinutes($ttlMinutes))) {
            return $cached;
        }

        return $this->updateMonthlyStats($schoolId, $year, $month);
    }
