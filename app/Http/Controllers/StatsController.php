...
class StatsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $expenseRaw = DB::table('transactions')
            ...
            ->get();
        // Resolve every user and their schools in TWO queries, before the loop.
        //
        // The loop below used to call User::find() per transaction row, then
        // $user->teacher->schools()->pluck() (or the assistant equivalent) per matching row —
        // one to three extra queries for every transaction in a twelve-month window, on every
        // dashboard render. Eager-loading the relations makes ->schools a loaded collection
        // rather than a fresh query each time.
        $userIds = collect($expenseRaw)->pluck('user_id')->filter()->unique()->values();
        ...
        foreach ($expenseRaw as $tx) {
            if ($tx->type === 'payment') {
                $user = $usersById->get($tx->user_id);
                if ($user && $user->role === 'teacher' && $user->teacher) {
                    $schools = $user->teacher->schools->pluck('id');
                ...
                if ($user && $user->role === 'assistant' && $user->assistant) {
                    $schools = $user->assistant->schools->pluck('id');
                    foreach ($schools as $sid) {
                        $expenseRows[] = [
                            'year' => (int) date('Y', strtotime($tx->payment_date)),
                            'month' => (int) date('m', strtotime($tx->payment_date)),
